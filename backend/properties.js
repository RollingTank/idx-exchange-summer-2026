...
router.get("/", async (req, res) => {
  try {
    ...
    if (minYearBuilt > maxYearBuilt) {
      return res.status(400).json({
        error: `Min Year Built must be Greater than Max Year Built`,
      });
    }

    const normalizedSortBy = sortBy ? String(sortBy).trim() : "";
    const normalizedSortOrder = sortOrder
      ? String(sortOrder).trim().toUpperCase()
      : "ASC";

    if (normalizedSortBy && !SORTABLE_COLUMNS.has(normalizedSortBy)) {
      return res.status(400).json({
        error: "Invalid sortBy value.",
      });
    }

    if (normalizedSortBy && !["ASC", "DESC"].includes(normalizedSortOrder)) {
      return res.status(400).json({
    ...
    if (minYearBuilt) {
      wheres.push("YearBuilt >= ?");
      queries.push(Number(minYearBuilt));
    }
    if (maxYearBuilt) {
      wheres.push("YearBuilt <= ?");
      queries.push(Number(maxYearBuilt));
    }

    const where_query =
      wheres.length > 0 ? `WHERE ${wheres.join(" AND ")}` : "";
    const count_query = `SELECT COUNT(*) as total FROM rets_property ${where_query}`;
    const order_query = normalizedSortBy
      ? `ORDER BY ${normalizedSortBy} ${normalizedSortOrder}`
      : "";
    const data_query = `SELECT * FROM rets_property ${where_query} ${order_query} LIMIT ? OFFSET ?`;
